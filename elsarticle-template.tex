...
While true that many biface types may not warrant the time and labour investment associated with 3D data collection and analysis, Gahagan bifaces do. Morphological attributes associated with axial twisting and beveling hold substantive analytical value beyond the current study, and the 3D data will eventually be employed within a variety of useful simulations akin to those used by \citet{RN20857}. Due to the amount of axial twisting and beveling, other recent morphometric approaches used to analyse bifaces do not work for Gahagan bifaces \citep{RN253,RN4143}. Although not expressly discussed above, the iterative refinement of the landmark configuration used in this study---and the one before it---will continue. As a means of expanding upon these efforts, the next iteration will include landmarks that articulate with a series of longitudinal cross-sections (Figure ~\ref{fig:beveling}, left).
...
Enlisting the reference geometry used in the development of the current landmark configuration, the measure is comprised of an angle that occurs between two linear vectors. To calculate the angle from the aligned mesh, two cross-sections are projected; one at the base located 1mm above the point at which the entirety of the cross-section is enclosed, and the other at a prescribed distance from the tip (Figure ~\ref{fig:beveling}, right). The cross-sections are split at the horizontal tangent of each edge, and a point is placed at the location where the spline (cross-section) is split. Two vectors are subsequently inserted, each enlisting the two points on each cross-section identified in the previous step, after which the angle between the two vectors can be calculated. It is not known whether this measure will correlate with those associated with retouch. However, should this angle be found to correlate with attributes related to size, spin torque, or other useful measures, it could hold value in future analyses.
